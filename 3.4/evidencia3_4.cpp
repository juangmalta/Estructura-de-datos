...
int busqueda_Binaria(vector<string> a, float k)//Complejidad de O(logn)
{
    int n = a.size();
    int l = 0;
    int r = n - 1;
...
int main()
{
  ...
  */
  ...
  cout << "Pasa IPS2"<<endl;
  for (int i = 0; i < vectorSeparado.size()-1; i++) //-1
  {

    if (i+1<vectorSeparado.size()) {

    }
    // cout<<vectorConvertido[i][3]<<endl; // Imprime linea
    if (vectorSeparado[i + 1][3] == vectorSeparado[i][3])
    {
  ...
  for (int i=0; i < vectorSeparado.size(); i++)
  {
    cout<<vectorConvertido[i][3]<<endl;
    int encuentra = busqueda_Binaria(ipLista, stol(vectorConvertido[i][3]));
